...
export interface RiskBreakdown {
  ...
  sdohScore: number;
  overallScore: number;
  conditions: { code: FhirCoding; hccCategory?: string; documented: boolean }[];
  sdohFactors: { factor: string; risk: 'low' | 'medium' | 'high' }[];
}
...
export interface PopulationMetrics {
  totalMembers: number;
  byRiskTier: { tier: RiskTier; count: number; percentage: number }[];
  avgRiskScore: number;

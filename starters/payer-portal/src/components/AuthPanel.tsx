...
export function AuthPanel({ token, onTokenChange }: AuthPanelProps) {
  const [username, setUsername] = useState('member@payer.local');
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-[var(--dxp-text-secondary)]">
          Login with a payer portal user to get a JWT token. All FHIR API calls will include the Bearer token.
        </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="member@payer.local" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-3 gap-2 text-[10px] text-[var(--dxp-text-muted)]">
          <div className="rounded border border-[var(--dxp-border)] p-2">
            <span className="font-bold block">Member</span>
            member@payer.local / member
          </div>
          <div className="rounded border border-[var(--dxp-border)] p-2">
            <span className="font-bold block">Provider</span>
            provider@payer.local / provider
          </div>
          <div className="rounded border border-[var(--dxp-border)] p-2">
            <span className="font-bold block">Care Manager</span>
            care-mgr@payer.local / care-mgr
          </div>
        </div>
